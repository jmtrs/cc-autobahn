...
cc-autobahn is an **instrument cluster** (Tauri v2) styled after the amber VFD display of the Mercedes W203, showing Claude Code and Codex token consumption. It runs on **macOS and Linux** as a menu-bar/tray icon (D24): left click shows/hides a frameless, transparent, always-on-top panel. On macOS it has no Dock, no Cmd+Tab (`ActivationPolicy::Accessory`); on Linux it hides from the launcher via `skipTaskbar` + `.desktop NoDisplay` (D56). It opens anchored under the icon by default, but D41 restored drag-to-move from the header/model-selector zones with a persisted manual override. Settings and the tray menu can reset it to the tray anchor.
...
Releases: `npm run release -- <patch|minor|major|X.Y.Z>` (scripts/release.mjs) — bumps the version in `package.json`/`tauri.conf.json`/`Cargo.toml`/`Cargo.lock` in sync, runs `cargo test`, commits, tags `vX.Y.Z`, pushes. The tag triggers `.github/workflows/release.yml`: two parallel jobs. `release-macos` re-gates on tests, builds the **unsigned** universal (arm64+x86_64) dmg on `macos-latest`, **publishes** the GitHub Release (not a draft — the Homebrew cask URL must be live when the tap update lands) and bumps the cask in `jmtrs/homebrew-tap` (`Casks/cc-autobahn.rb`, skipped if the `HOMEBREW_TAP_TOKEN` secret is unset). `release-linux` builds `.deb`/`.rpm`/`.AppImage` on `ubuntu-22.04` via `tauri-apps/tauri-action` and publishes them to the same release, with an explicit `upload-artifact` guard against tauri-action#973 (D59). No signing (D34); process rationale in D35. CI (`.github/workflows/ci.yml`) runs `check-macos` and `check-linux` in parallel on every push/PR — Linux runs fmt+clippy+test+build+frontend logic tests, not visual regression (D58).

## Architecture (two layers)

- **Rust backend (`src-tauri/`)** — responsible for **all I/O**, never blocks the UI. Each sensor is a directory module, split by concern: `engine/` (`mod.rs`: `detect` global ccusage → npx → bunx + poll loop; `blocks.rs`: `poll_once`/`ccusage blocks --active --json` at **slow cadence 15 s**, D13; `install.rs`: `install_bun`, fire-and-forget — spawns a `std::thread` for the actual Bun install and reports via `install-progress`/`install-succeeded`/`install-failed` events instead of blocking the command handler, D36; `history.rs`: `history_daily`/`ccusage claude daily --json`, **on-demand** cadence, D33), `burn/` (`zulu.rs`: Zulu timestamp parsing; `parser.rs`: `TurnState`/`process_line`, pure turn-calc logic; `tail.rs`: JSONL file tail → `tok/s` per response, D17/D27), `sensor/` (`mod.rs`: shared paths + tail watcher, plus D51's `context_used_pct`/`cache_hit_pct` derived from the statusline's `context_window` field; `statusline_bin.rs`: the `statusline` CLI entrypoint; `install.rs`: settings.json auto-install/uninstall + `refresh_if_stale`, a background-thread self-refresh of the installed binary copy on every startup, D36), `permission/` (`mod.rs`: FIFO `PendingQueue` + Unix-socket listener thread + `permission_approve`/`permission_deny` commands, provider-namespaced since D49; `hook_bin.rs`: the `permission-hook` CLI entrypoint, blocks on the socket for a decision and fails open on any failure; `install.rs`: settings.json `hooks.PermissionRequest` array merge/unmerge + `refresh_if_stale`, same shape as `sensor::install` but array-based; `codex_install.rs`: the Codex-native equivalent — owns a `hooks.json` entry with atomic merge/backup/rollback, D49; `always_allow.rs`: Claude's native `updatedPermissions` echo plus the legacy exact-rule/session-memory fallback, and Codex's exact process-memory Always Allow key, D44/D49), `providers/` (`mod.rs`: provider-neutral contracts — `ProviderId::{Claude,Codex}`, `SourceQuality`, health/turn-rate shapes shared code consumes instead of provider wire formats, D45; `claude.rs`: Claude adapter wiring the existing engine/burn/sensor lifecycle into the shared contracts; `codex/mod.rs`: Codex adapter entry; `codex/rollout.rs`: defensive discovery/tail of Codex rollout JSONL under `CODEX_HOME`, D46; `codex/app_server.rs`: owned `codex app-server --stdio` child, the official Codex account/rate-limit sensor, D48), `pathfix.rs` (PATH hardening at GUI startup — prepends Homebrew/Bun paths when present on disk, since a Finder/Dock launch inherits launchd's bare PATH, D36), `tray_icon.rs` (tray icon progress ring, D30, plus the `alert`/`pending_permission` blink states and the dual-provider "most urgent candidate" summary), `window.rs` (PIN state, hide-on-blur, panel positioning), `tray.rs` (menu-bar menu/icon/click).
...
**Three continuous Claude sensors + one on-demand fetch, four cadences (D13/D33), plus one event-driven gate — Codex runs the equivalent shapes through its own adapters:** ccusage `blocks` = slow poll (cost/projection); JSONL tail = event per turn (`tok/s`, plus D51's context-fill/cache-hit percentages); statusline = push (official `rate_limits` data); ccusage `claude daily` = on-demand, fetched only when the History/Limits MFD page opens; the `PermissionRequest` hook (D49) is neither poll nor push — it's a blocking request/response over a Unix socket, one connection per tool call that needs a decision, shared FIFO across both providers. Codex mirrors this with its own rollout JSONL tail (D46) and owned App Server account sensor (D48) instead of ccusage/statusline.
...
**Permission hook (D42/D49) — approve/deny from the cluster:** unlike the statusLine sensor, a Claude Code or Codex hook is *synchronous* — the CLI blocks the tool call until the hook process exits. The file+poll pattern above can't answer that, so this uses a real `std::os::unix::net::UnixListener` socket (`~/.claude/cc-autobahn/permission.sock`): the hook blocks on it waiting for a human's Approve/Deny click, and prints nothing (letting the CLI fall back to its own terminal prompt) if cc-autobahn isn't running or nobody answers in time. Requests are provider-discriminated so identical IDs from Claude and Codex never collide in the shared FIFO queue (D44/D49). Opt-in from the Settings page, not auto-installed like the statusline sensor.

Target flow: the backend emits events (`blocks-update`, `burn-tick`, `sensor-update`, `rate-limit-update`, `account-usage-update`, `provider-health`, `app-engine-missing`, `permission-pending`/`permission-resolved`, `desktop-permission-pending`/`desktop-permission-resolved`, and more) that the frontend listens to and renders. Full list and data flow in `docs/ARCHITECTURE.md`.
...
| PRND selector         | Provider-native active-model family lit up |
...
- **Tray/menu-bar (D24)**: show/hide/position live in `src-tauri/src/window.rs`; the menu/icon/"Quit"/click-to-toggle live in `src-tauri/src/tray.rs` (`TrayIconBuilder`, `tray-icon` feature of the `tauri` crate itself, no new plugin); `main.rs` just wires the two together in `.setup()`. The **icon itself** is a progress ring redrawn at runtime by `tray_icon.rs` (D30), called from `engine/`/`sensor/` on every new data point — not a static PNG (that PNG, `icons/tray-icon-template.png` on macOS / `icons/tray-icon-linux.png` on Linux, only remains as the initial icon before the first redraw). **On macOS always use `TrayIcon::set_icon_with_as_template()`, never plain `set_icon()`** — `set_icon()` doesn't preserve macOS's "template" flag across calls and the icon gets repainted as fixed black instead of adapting to light/dark mode (real bug, D30). On Linux use plain `set_icon()` and paint amber RGB — AppIndicator has no template concept, alpha-only renders black (D55); `write_pixel` in `tray_icon.rs` is the single cfg-forked pixel write. `ActivationPolicy::Accessory` only on macOS (`#[cfg(target_os = "macos")]`); the rest of the tray API is cross-platform. **Linux preconditions** (D57): requires a tray-supporting desktop (GNOME needs the AppIndicator extension) and a compositing WM for the transparent panel (Mutter/KWin/wlroots/picom).
