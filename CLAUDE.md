...
**Current state: Phases 0–6 done** (real checklist in `docs/ROADMAP.md`, the rationale for each piece in `docs/DECISIONS.md` D1–D42). Only two optional/future items remain (Bun sidecar, Windows/Linux). The three continuous sensors (`engine`, `burn`, `sensor`) run on dedicated threads and are wired into the display: speedometer (physical spring, D18), segment bar (estimated `EST` or official data with priority, D23), PRND selector (D7), PACE/AUTO footer (D28), PIN button (D26). The display is a 4-page MFD cycled by one button (`#mfd-btn`, header, D33): Page 0 trip computer (the above), Page 1 daily history (`engine::history::history_daily`, on-demand cadence), Page 2 weekly rate-limit + per-model cost + burn rate, Page 3 settings (`localStorage`). With no engine detected, the frontend paints the "CHECK ENGINE" overlay with a button that installs Bun on its own (`engine::install_bun`, D9) and relaunches the engine without restarting the app; the installer runs on a background thread with staged progress events instead of blocking the UI (D36). The dual binary is also Claude Code's `statusLine` command (auto-installable, D19–D22), and its installed binary copy self-refreshes on every startup so an old release doesn't leave it stale (D36). A fourth, opt-in module (`permission/`) self-installs as Claude Code's `PermissionRequest` hook (D42): any session's permission prompts surface in the cluster with Approve/Deny buttons, resolved over a Unix domain socket that blocks the hook process until a human clicks — fails open (prints nothing, Claude Code falls back to its terminal prompt) if the GUI isn't running. PATH is hardened at GUI startup so a Finder/Dock launch still finds an already-installed engine (D36). The tray icon (D24) is a progress ring redrawn at runtime, not a static PNG (D30). `cargo test` 42/42, `cargo clippy` clean.
...
Backend: `cargo test` (42 tests, in `src-tauri/`) + `cargo clippy` clean. Frontend: no tests or linter configured.
...
- **Frontend (webview, `index.html` + `src/`)** — presentation only, no system I/O; receives data via Tauri IPC/events. `src/style.css` = amber skin; `src/main.js` is a thin entrypoint that wires the widget modules under `src/modules/` (clock, speedometer, trip-computer, footer-metric, redline, engine-overlay, sensor-consent, permission-gate, permission-consent, pin-button, ipc-events, plus shared `format.js`/`telemetry-state.js`) plus the MFD page modules (`mfd-nav.js`, `mfd-settings.js`, `history-data.js`, `history-page.js`, `limits-page.js`, `settings-page.js`, D33). `redline.js` (D37) drives the PACE/AUTO threshold reaction (screen tint, segment bar, footer/speedometer kick) — pure CSS/JS, no backend involvement.
