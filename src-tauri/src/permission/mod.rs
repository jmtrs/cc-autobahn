...
/// GUI → hook, one line: `{"decision":"allow"}` / `{"decision":"deny"}`.
...
impl PendingPayload {
    fn from_slot(slot: &PendingSlot, pending_count: usize) -> Self {
        PendingPayload {
            id: slot.request.prompt_id.clone(),
            ...
            pending_count,
            always_allow_available: matched_field(
                &slot.request.tool_name,
                &slot.request.tool_input,
            )
            .is_some(),
...
fn emit_state(app: &AppHandle, queue: &PendingQueue) {
    ...
    match payload {
        ...
        None => {
            ...
            let _ = app.emit("permission-resolved", ());
        }
    }
}
...
pub fn permission_approve_always(app: AppHandle, id: String) -> Result<(), String> {
    let queue = app.state::<PendingQueue>();
    let (session_id, tool_name, cwd, matched) = {
        let q = queue.lock().unwrap();
        let slot = q
            .iter()
            .find(|s| s.request.prompt_id == id)
            .ok_or("no such pending permission request (it may have timed out)")?;
        let matched = matched_field(&slot.request.tool_name, &slot.request.tool_input)
            .ok_or("always-allow isn't available for this tool")?;
        (
            slot.request.session_id.clone(),
            slot.request.tool_name.clone(),
            slot.request.cwd.clone(),
            matched,
        )
    };

    let persist_result = always_allow::remember(&app, &session_id, &tool_name, &cwd, &matched);
    resolve(&app, &id, Decision::Allow)?;
    persist_result
}
...
fn resolve(app: &AppHandle, id: &str, decision: Decision) -> Result<(), String> {
    let queue = app.state::<PendingQueue>();
    let slot = {
        let mut q = queue.lock().unwrap();
        let idx = q
            .iter()
            .position(|s| s.request.prompt_id == id)
            .ok_or("no such pending permission request (it may have timed out)")?;
        q.remove(idx)
            .expect("idx just found via position on the same locked queue")
    };
...
fn handle_connection(stream: UnixStream, queue: PendingQueue, app: AppHandle) {
    ...
    if let Some(matched) = matched_field(&request.tool_name, &request.tool_input) {
        ...
        if always_allow::is_remembered(
            ...
            respond(stream, Decision::Allow);
            return;
        }
    }

    let id = request.prompt_id.clone();
    ...
    match outcome {
        ...
        WaitOutcome::TimedOut | WaitOutcome::PeerGone => {
            let still_queued = {
                let mut q = queue.lock().unwrap();
                let existed = q.iter().any(|s| s.request.prompt_id == id);
                q.retain(|slot| slot.request.prompt_id != id);
                existed
            };
...
#[cfg(unix)]
fn respond(stream: UnixStream, decision: Decision) {
    let msg = DecisionMsg { decision };
...
mod tests {
    ...
    fn decided_immediately() {
        ...
        tx.send(Decision::Allow).unwrap();
        match wait_outcome(&rx, &gone_rx, SHORT) {
            WaitOutcome::Decided(Decision::Allow) => {}
    ...
    fn times_out_when_nothing_arrives() {
        let (_tx, rx) = mpsc::channel::<Decision>();
        let (_gone_tx, gone_rx) = mpsc::channel::<()>();
        assert!(matches!(
    ...
    fn decision_wins_the_race_with_peer_gone() {
        ...
        match wait_outcome(&rx, &gone_rx, SHORT) {
            WaitOutcome::Decided(Decision::Deny) => {}
